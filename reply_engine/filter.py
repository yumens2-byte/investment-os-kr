...
def is_korean_dominant(text: str, threshold: float = 0.3) -> bool:
    """문자 중 한글 비율을 검사한다. 이모지/기호뿐이면 언어 판정을 보류한다."""
    body = re.sub(r"@\w+", "", text or "")
    letters = re.findall(r"[A-Za-z가-힣ㄱ-ㅎㅏ-ㅣ]", body)
    if not letters:
        return True
    korean = sum(bool(re.fullmatch(r"[가-힣ㄱ-ㅎㅏ-ㅣ]", ch)) for ch in letters)
    return korean / len(letters) >= threshold
